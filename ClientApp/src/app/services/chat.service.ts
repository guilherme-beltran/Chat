...
export class ChatService {
  ...
  constructor(private httpClient: HttpClient, private modalServices: NgbModal) { }

  registerUser(user: User){
    return this.httpClient.post(`${environment.apiUrl}chat`, user, {responseType: 'text'});
  }

  createConnection(){
    this.chatConnection = new HubConnectionBuilder()
    .withUrl(`${environment.urlConnection}/hubs/chat`).withAutomaticReconnect().build();
    ...
    this.chatConnection.on('OpenPrivateChat', (newMessage: Message) => {
      ...
      const modalRef = this.modalServices.open(PrivateChatComponent);
